...
- 默认按这个顺序执行：**检查是否已安装 → 首次安装或更新 → 检查配置 → 需要时补配置 → 启动 → 立即输出二维码 → 告知后续使用方式**。
- 只有在以下情况才询问用户：
  - 缺少完成 `pair` 所必须的邮箱
...
**只要 `coderelay start` 已经成功启动，并且本地连接信息齐全，就必须立即执行一次：**
...
- 默认流程是：检查 → 安装/更新 → 检查配置 → 必要时配对 → 启动 → 立即输出二维码/连接方式 → 告知后续使用方式
